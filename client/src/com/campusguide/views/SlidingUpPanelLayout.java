...
public class SlidingUpPanelLayout extends ViewGroup {
	...
	/**
	 * Lock the touch event while the inner view are dragging horizontally
	 */
	private boolean mIsDragingHorizontally;

	private boolean mIsDragPeekingViewOnly;
	...
	private boolean mIsSlidableViewVisible = false;
	...
	public void setDragPeekingViewOnly(boolean enable) {
		this.mIsDragPeekingViewOnly = true;
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		/*
		 * final int widthMode = MeasureSpec.getMode(widthMeasureSpec); final
		 * int widthSize = MeasureSpec.getSize(widthMeasureSpec); final int
		 * heightMode = MeasureSpec.getMode(heightMeasureSpec); final int
		 ...
		 * 
		 * if (childCount > 2) { Log.e(TAG,
		 * "onMeasure: More than two child views are not supported."); } else if
		 * (getChildAt(1).getVisibility() == GONE) { panelHeight = 0; }
		 * 
		 * // We'll find the current one below. mSlideableView = null; mCanSlide
		 * = false;
		 * 
		 * // First pass. Measure based on child LayoutParams width/height. for
		 * (int i = 0; i < childCount; i++) { final View child = getChildAt(i);
		 * final LayoutParams lp = (LayoutParams) child.getLayoutParams();
		 * 
		 * int height = layoutHeight; if (child.getVisibility() == GONE) {
		 * lp.dimWhenOffset = false; continue; }
		 * 
		 * if (i == 1) { lp.slideable = true; lp.dimWhenOffset = true;
		 * mSlideableView = child; hidePane(); mCanSlide = true; } else { height
		 * -= panelHeight; }
		 * 
		 * int childWidthSpec; if (lp.width == LayoutParams.WRAP_CONTENT) {
		 * childWidthSpec = MeasureSpec.makeMeasureSpec(widthSize,
		 * MeasureSpec.AT_MOST); } else if (lp.width ==
		 * LayoutParams.MATCH_PARENT) { childWidthSpec =
		 * MeasureSpec.makeMeasureSpec(widthSize, MeasureSpec.EXACTLY); } else {
		 * childWidthSpec = MeasureSpec.makeMeasureSpec(lp.width,
		 * MeasureSpec.EXACTLY); }
		 * 
		 * int childHeightSpec; if (lp.height == LayoutParams.WRAP_CONTENT) {
		 * childHeightSpec = MeasureSpec.makeMeasureSpec(height,
		 * MeasureSpec.AT_MOST); } else if (lp.height ==
		 * LayoutParams.MATCH_PARENT) { childHeightSpec =
		 * MeasureSpec.makeMeasureSpec(height, MeasureSpec.EXACTLY); } else {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		 ...
		 * 
		 * if (mFirstLayout) { mSlideOffset = mCanSlide &&
		 * mPreservedExpandedState ? 0.f : 1.f; }
		 * 
		 * for (int i = 0; i < count; i++) { final View child = getChildAt(i);
		 * if (child.getVisibility() != GONE) { final LayoutParams lp =
		 * (LayoutParams) child.getLayoutParams();
		 * 
		 * final int width = child.getMeasuredWidth(); final int height =
		 * child.getMeasuredHeight();
		 * 
		 * int childLeft = parentLeft; int childTop = parentTop;
		 * 
		 * if(i==1) childTop = childTop + height - mPanelHeight;
		 * 
		 * child.layout(childLeft, childTop, childLeft + width, childTop +
		 * height); } }
		 * 
		 * if (mFirstLayout) { updateObscuredViewVisibility(); }
		 * 
		 * mFirstLayout = false;
		 */
	...
	private boolean isPeekingViewHit(int x, int y) {
		View v = mDragView != null ? mDragView : mSlideableView;
		if (v == null)
			return false;
		int[] viewLocation = new int[2];
		...
		return screenX >= viewLocation[0]
				&& screenX < viewLocation[0] + v.getWidth()
				&& screenY >= viewLocation[1]
				&& screenY < viewLocation[1] + this.mPanelHeight;
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (action == MotionEvent.ACTION_CANCEL
			...
			mDragHelper.cancel();
			mIsDragingHorizontally = false;
			Log.e("event", "2");
			return false;
		}

		if (mIsDragingHorizontally) {
			mDragHelper.cancel();
			Log.e("event", "3");
			return false;
		}
		...
		case MotionEvent.ACTION_DOWN: {
			...
			mInitialMotionX = x;
			mInitialMotionY = y;
			mDragViewHit = mIsDragPeekingViewOnly ? isPeekingViewHit((int) x,
					(int) y) : isDragViewHit((int) x, (int) y);
		...
		Log.e("event", "6" + interceptTap + interceptForDrag);
	...
	public void showPane() {
		...
		mIsSlidableViewVisible = true;
		mPanelHeight = mPanelHeightBak;
		View slidingPane = getChildAt(1);
